<?php
    ...
    if ($datef == "" || $datet == "") {
        echo "
            <script>
                window.alert('Empty Date Input!');
                window.location.href = 'transfer_reports'
            </script>
        ";
    }else if ($datef == "0000-00-00" || $datet == "0000-00-00") {
        echo "
            <script>
                window.alert('Empty Date Input!');
                window.location.href = 'transfer_reports'
        ...
        if ($datef == $datet) {
            $my_project_header_title = "Stock-Transfer Reports: ".date("M d, Y", strtotime($datef))." / ".$branch_row['gy_branch_name'];
        }else{
            $my_project_header_title = "Stock-Transfer Reports: ".date("M d", strtotime($datef))." - ".date("M d, Y", strtotime($datet))." / ".$branch_row['gy_branch_name'];
        }

        $my_query=$link->query("Select * From `gy_stock_transfer` Where `gy_branch_id`='$my_branch' AND date(`gy_transfer_date`) BETWEEN '$datef' AND '$datet'  Order By `gy_transfer_date` ASC");

        $count_results=$my_query->num_rows;
    }else{

        if ($datef == $datet) {
            $my_project_header_title = "Stock-Transfer Reports: ".date("M d, Y", strtotime($datef));
        }else{
            $my_project_header_title = "Stock-Transfer Reports: ".date("M d", strtotime($datef))." - ".date("M d, Y", strtotime($datet));
        }

        $my_query=$link->query("Select * From `gy_stock_transfer` Where date(`gy_transfer_date`) BETWEEN '$datef' AND '$datet'  Order By `gy_transfer_date` ASC");
                ...
                <div class="col-md-12">
                    ...
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            Stock-Transfer Data Table <b><?php echo 0+$count_results; ?></b> result(s) <a href="print_tsummary?datef=<?php echo $datef; ?>&datet=<?php echo $datet; ?>&mode=date_search&branch=<?php echo $my_branch; ?>" onclick="window.open(this.href, 'mywin',
                                        ...
                                        <div class="modal fade" id="void_<?php echo $transfer_row['gy_transfer_id']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <!-- <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button> -->
                                                        <h4 class="modal-title" id="myModalLabel"><i class="fa fa-trash-o fa-fw"></i> Void Stock-Transfer Data </h4>
                                                    </div>
                                                    <div class="modal-body">
